...
- Every monster nest on the active map now spawns a deterministic “stand-in” that uses its real atlas animation (`monsters.json` → `TypeId` → `m###` cr9). That gives instant visual feedback about which creatures populate each nest while keeping the Sprites sorted with the tile renderer. The previews reuse the same frame slicing logic as the viewer so any discrepancy between JSON and atlas data is obvious on the world scene.
